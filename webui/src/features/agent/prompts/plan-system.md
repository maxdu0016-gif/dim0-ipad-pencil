...
- **mini-app** (`note_type="mini-app"` — the default custom-rendered artifact: chart, dashboard, diagram, flashcard, interactive control, …): call `learn_generate_mini_app` first, then follow its instructions when writing the note. When based on an existing note, create it beside the source using `near` rather than replacing the source or placing it at the board's bottom. The source is validated via sucrase and rejected with line/col if malformed — fix and retry once if rejected.
